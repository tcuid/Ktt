...
jQuery.cookie = function(name, value, options) {
			...
			$(function() {
				var COOKIE_NAME = "setCookie";
				if($.cookie(COOKIE_NAME)) {
					$(".layer").hide();
					$("#globalAd").hide();
				} else {
					var setCookieH = $(window).height();
					var setCookieW = $(window).width();
					$(".layer").show();
					$("#globalAd").css({
						'top': setCookieH / 2 - $("#globalAd").height() / 2,
						'left': setCookieW / 2 - $("#globalAd").width() / 2
					});
					$("#globalAd").slideDown(300, function() {
						setTimeout("closeGloableAd()", '300000');
					});
					$.cookie(COOKIE_NAME, "setCookie", {
						path: '/',
						expires: 1
					});
				}
